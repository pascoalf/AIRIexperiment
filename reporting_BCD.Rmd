...
quality(diagnosis_rules_dependent)$improvement <- interestMeasure(diagnosis_rules_dependent,
                                                             measure = "improvement")
#
#diagnosis_rules_dependent %>% DATAFRAME() %>% kable()
```
...
#diagnosis_rules_non_redundant_improv %>% kable()
...
#diagnosis_rules_non_redundant_mutualInfo %>% kable()
...
#diagnosis_rules_non_redundant_complex %>% kable()
```
...
gridExtra::grid.arrange(
  diagnosis_all_non_redundant %>% 
    ...
    geom_jitter(height = 0, width = 0.1, aes(col = RHS)) +
    theme_classic() + 
...
## Redundancy by Jaccard


Jaccard was used to measure the overlap, i.e. redundancy, between itemsets in the same rule set.
